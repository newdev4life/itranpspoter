...
export function Upload({ onStartUpload }: UploadProps) {
    ...
    const handleFetchProviders = async () => {
        ...
        if (!currentAppleId || !currentPassword) {
            setProviderError(t('upload.error.fill_cred_first'))
            return
        }

        if (useNewCredential && !validateEmail(currentAppleId)) {
            setProviderError(t('upload.error.invalid_id'))
            return
        }
        ...
        try {
            ...
            if (result.success && result.providers) {
                ...
                setProviderError(result.errorMessage || t('upload.error.fetch_failed'))
            }
        } catch (error) {
            setProviderError(t('upload.error.fetch_error'))
        }
    ...
    const saveCurrentCredential = async () => {
        ...
        if (!validateEmail(appleId)) {
            alert(t('upload.error.invalid_id'))
            return
        }
        try {
            ...
            // Refresh list
            loadCredentials()
            alert(t('upload.cred_saved'))
        } catch (e) {
            console.error(e)
            alert(t('upload.save_failed'))
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!finalAppleId || !finalPassword) {
            alert(t('upload.error.incomplete_cred'))
            return
        }
        ...
        if (!selectedProvider) {
            if (providers.length === 0) {
                ...
                alert(t('upload.error.no_provider'))
                return
            }
        }
            ...
            <div className="upload-card">
                ...
                <div className="form-group">
                    ...
                    <div className="input-row">
                        <input
                            ...
                            value={ipaPath}
                            placeholder={t('upload.browse_placeholder')}
                            readOnly
                ...
                <div className="form-group">
                    <div className="form-label-row">
                        ...
                        {credentials.length > 0 && (
                            ...
                            >
                                {useNewCredential ? `${t('upload.select_saved')} ∨` : t('upload.use_new')}
                            </a>
                        )}
                    </div>
                            ...
                            <input
                                ...
                                onBlur={() => {
                                    if (appleId && !validateEmail(appleId)) {
                                        setAppleIdError(t('upload.invalid_email'))
                                    }
                                }}
                        ...
                        <div className="input-row">
                            ...
                            {useNewCredential && appleId && password && (
                                <button
                                    ...
                                    title={t('upload.save_credential')}
                                >
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="form-group">
                    ...
                    {providers.length > 0 && (
                        ...
                        >
                            <option value="">{t('upload.select_team')}</option>
                            {providers.map(p => (
